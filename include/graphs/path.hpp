...
// - set_sentinel() uses logical index -1 to address padding.
...
public:
    ...
    // -------------------- Counts --------------------------------------
    inline std::size_t count_by_color(std::optional<bool> c = std::nullopt) const {
        const std::size_t occ_count = occ_.count();
    ...
    inline std::size_t unhappy_count() const { return unhappy_mask_cache_.count(); }

    // Uniform pick over unoccupied vertices (kth-zero based).
    template<class URBG>
    inline std::size_t get_unoccupied(URBG& rng) const noexcept {
    ...
    template<class URBG>
    inline std::size_t get_unhappy(URBG& rng) const noexcept {
...
private:
    ...
    uint8_t local_frustration(std::size_t v) const { return (disagree_right(v) + disagree_left(v)); }
    bool disagree_left(std::size_t v) const { return occ_[v] && occ_[v-1] && (col_[v] != col_[v-1]); }
    bool disagree_right(std::size_t v) const { return occ_[v] && occ_[v+1] && (col_[v] != col_[v+1]); }
    uint8_t neighbors(std::size_t v) const { return static_cast<uint8_t>(occ_[v-1]) + static_cast<uint8_t>(occ_[v+1]); }

    // Only called for idx <= B-1
    void local_unhappy_reset(std::size_t idx) {
    ...
    void local_unhappy_update(std::size_t idx) {
